...
export const getAllCreditPurchaseController = async (req, res, next) => {
  ...
  try {
    let query = `
            SELECT 
                i.id, i.customer_name, i.customer_gstno, i.total_price, i.currency, i.invoice_link, i.transaction_id,i.payment_id, i.invoice_date,
  ...
  try {
    ...
    let query = `
            SELECT 
                ...
                a.city, a.state, i.no_of_credits, i.transaction_id, i.payment_id, i.subscription_id, i.invoice_no, i.type
            FROM customer_invoices i
            JOIN users u ON i.user_id = u.user_id
            JOIN addresses a ON i.address_id = a.id
            WHERE i.type = 'credit'
