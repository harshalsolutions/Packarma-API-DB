...
export const exportAllReferralsController = async (req, res, next) => {
    try {
        const [referralsRows] = await pool.query(`
            SELECT r.id, r.account_created, r.redeem_status, r.subscription_completed, rc.code, u.firstname as referred_firstname, u.lastname as referred_lastname, u.email as referred_email, u2.firstname as referrer_firstname, u2.lastname as referrer_lastname, u2.email as referrer_email, r.createdAt
            FROM referrals AS r 
            JOIN referral_codes AS rc ON r.referral_code_id = rc.id 
            JOIN users AS u ON r.referred_user_id = u.user_id
            JOIN users AS u2 ON rc.user_id = u2.user_id
