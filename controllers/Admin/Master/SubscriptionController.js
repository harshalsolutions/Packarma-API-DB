...
export const createSubscriptionController = async (req, res, next) => {
  try {
    ...
    const query =
      "INSERT INTO subscriptions (type, credit_amount, duration, benefits) VALUES (?, ?, ?, ?, ?)";
...
export const deleteSubscriptionController = async (req, res, next) => {
  try {
    ...
    const query =
      "UPDATE subscriptions SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND deleted_at IS NULL";
...
export const getAllCurrencyController = async (req, res, next) => {
  try {
    const { id } = req.params;
    const query =
      "SELECT DISTINCT currency FROM subscriptions_prices WHERE subscription_id = ?";
    const [currencyRows] = await pool.query(query, [id]);

    const usedCurrencies = currencyRows.map((row) => ({
      code: row.currency,
      symbol: currencyData[row.currency]?.symbol || "",
      name: currencyData[row.currency]?.name || "",
    }));

    const notUsedCurrencies = Object.keys(currencyData)
    ...
    res.json(
      new ApiResponse(
        200,
        { used: usedCurrencies, notUsed: notUsedCurrencies },
...
export const getSubscriptionPriceController = async (req, res, next) => {
  try {
    ...
    const query =
      "SELECT * FROM subscriptions_prices WHERE subscription_id = ?";
    const [rows] = await pool.query(query, [id]);

    if (!rows.length)
      throw new CustomError(404, "Subscription price not found");

    res.json(
      new ApiResponse(200, rows, "Subscription price retrieved successfully"),
    );
  } catch (error) {
    next(new CustomError(500, error.message));
  }
...
export const addSubscriptionPriceController = async (req, res, next) => {
  try {
    ...
    const query =
      "INSERT INTO subscriptions_prices (subscription_id, price, currency, status) VALUES (?, ?, ?, ?)";
    const [result] = await pool.query(query, [
      subscription_id,
      price,
      currency,
...
export const updateSubscriptionPriceController = async (req, res, next) => {
  try {
    ...
    res.json(
      new ApiResponse(200, null, "Subscription price updated successfully"),
    );
  } catch (error) {
    next(new CustomError(500, error.message));
  }
...
export const deleteSubscriptionPriceController = async (req, res, next) => {
  try {
    ...
    res.json(
      new ApiResponse(200, null, "Subscription price deleted successfully"),
    );
  } catch (error) {
    next(new CustomError(500, error.message));
  }
